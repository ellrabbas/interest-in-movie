    ...
    try {
        ...
        if (movie) {
            ...
            await Promise.all(artists.map(async artist => {
                artist.movies = artist.movies.filter(movieId => movieId.toString() !== req.params.id);
                await artist.save();
            }));
...
const deleteAllMovies = asyncHandler(async (req, res) => {
    try {
        ...
        const artists = await Artist.find({ movies: { $in: deletedMovies.map(movie => movie._id) } });

        await Promise.all(artists.map(async artist => {
            artist.movies = artist.movies.filter(movieId => !deletedMovies.find(deletedMovie => deletedMovie._id.equals(movieId)));
            await artist.save();
        }));
...
const createArtist = asyncHandler(async (req, res) => {
    try {
        ...
        const findOrCreateArtist = async (artistData) => {
            const { fullName, occupation, image, gender, nationality, age, zodiacSign } = artistData;
    ...
    try {
        ...
        if (artist) {
            ...
            const moviesToUpdate = await Movie.find({
                $or: [
                    { casts: artist._id },
                    { directors: artist._id }
                ]
            });


            await Promise.all(moviesToUpdate.map(async (movie) => {
                movie.casts = movie.casts.filter(cast => cast.toString() !== req.params.id);
                movie.directors = movie.directors.filter(director => director.toString() !== req.params.id);
                await movie.save();
            }));
