...
function Table2({ data, users, onEditCategory }) {
    ...
    const Rows = (data, index, users, onEditCategory) => {
        return (
            <tr key={index}>
                {users ? (
                    <>
                        ...
                        <td className={`${Text}`}>
                            {data?._id ? data._id : "2RT75T8"}
                        </td>
                        <td className={`${Text}`}>
                            {data._cratedAt ? data._cratedAt : "16, April 2024"}
                        </td>
                    ...
                    <>
                        ...
                        <td className={`${Text} flex-rows gap-2 mt-2`}>
                            <button onClick={() => onEditCategory(data)} title="Edit" className="border border-dry text-dry hover:bg-dry hover:text-main transitions flex-rows gap-2 rounded-full p-1">
                                <MdEdit size={15} />
                            </button>
                            <button title="Delete" className="border border-dry text-dry hover:bg-dry hover:text-main transitions flex-rows gap-2 rounded-full p-1">
                                <MdDelete size={15} />
            ...
            <div className="overflow-y-scroll max-h-content scrollbar-hide relative w-full ">
                ...
                >
                    ...
                    <tbody className="divide-y divide-gray-800">
                        {currentItems.map((data, index) => Rows(data, index, users, onEditCategory))}
                    </tbody>
                </table>
            </div>
